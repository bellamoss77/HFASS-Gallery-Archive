...
items.forEach((e, i) => {
    ...
    const icon = e.querySelector('.fa-angle-down');
    ...
    header.addEventListener('click', () => {
        ...
        if (isOpening) {
            ...
            gsap.to(icon, { duration: 0.2, rotate: '180deg', ease: 'power2.inOut' })
        } else {
            gsap.to(content, { height: 0, duration: 0.5, ease: 'power2.in' });
            gsap.to(icon, { duration: 0.2, rotate: '0deg', ease: 'power2.inOut' });
        }
        ...
        if (currentItem !== null & currentItem !==i) {
            ...
            gsap.to(prevIcon, { duration: 0.2, rotate: '0deg', ease: 'power2.inOut' });
...
subItems.forEach((subItem) => {
    ...
    subHeader.addEventListener('click', (event) => {
        ...
        if (isSubOpening) {
            ...
            gsap.to(subIcon, { duration: 0.2, rotate: '180deg', ease: 'power2.inOut' });
        } else {
            gsap.to(subContent, { height: 0, duration: 0.5, ease: 'power2.in' });
            gsap.to(subIcon, { duration: 0.2, rotate: '0deg', ease: 'power2.inOut' });
        }
    });
});
